...
class Customer {
    ...
    public Customer(String email, String phone, String userName, String password, double balance, Bank bank,
            Account account) {
        this.email = email;
        this.phone = phone;
...
public class BankApp {
    ...
    public void transferAmount() {
        ...
        if (sender != null && receiver != null) {
            if (sender.getBalance() >= amount) {
                ...
                // Record transaction details for sender
                if (sender.getTransactionHistory() == null) {
                    sender.setTransactionHistory(new ArrayList<>());
                }
                sender.getTransactionHistory().add("Transfer to " + receiverEmail + ": $" + amount);

                // Record transaction details for receiver
                if (receiver.getTransactionHistory() == null) {
                    receiver.setTransactionHistory(new ArrayList<>());
                }
                receiver.getTransactionHistory().add("Transfer from " + senderEmail + ": $" + amount);
    ...
    public void getMiniStatement() {
        ...
        Customer customer = customers.get(email);
        if (customer != null) {
            System.out.println("Mini Statement for " + customer.getUserName() + ":");

            // Check if transactionHistory is not null before accessing it
            if (customer.getTransactionHistory() != null) {
                List<String> transactionHistory = customer.getTransactionHistory();
    ...
    public static void main(String[] args) {
        BankApp bankSystem = new BankApp();
        while (true) {
            ...
            System.out.println("5. Check Mini Statement");
